...
class Hasher:
    ...
    def get_index(self, board:chess.Board, square:chess.Square):
        piece = board.piece_at(square)
        if piece is None:
            return -1

        if piece.color == chess.WHITE:
            return 64 * offsets[piece.piece_type] + square
        else:
            return 6 * 64 + 64 * offsets[piece.piece_type] + square

    def get_offset(self, piece:chess.Piece):
        if piece.color == chess.WHITE:
            return 64 * offsets[piece.piece_type]
        else:
            return 6 * 64 + 64 * offsets[piece.piece_type]

    def get_pos_hash(self, board:chess.Board):
        res = 0
        for i in range(64):
            piece = board.piece_at(square=i)
            if piece is not None:
               res ^= self.array[self.get_index(board, i)]
        return res
    ...
    def get_hash(self, board:chess.Board):
        return (get_pos_hash(board) ^ self.hash_castling(board) ^
                self.hash_ep_square(board) ^ self.hash_turn(board))

    def updated_position_hash(self, board:chess.Board, move:chess.Move):
        # return value to be xored with position hash
        res = 0

        # remove myself from current cell
        res ^= self.array[self.get_index(board, move.from_square)]
        # remove enemy from dest cell
        res ^= self.array[self.get_index(board, move.to_square)]
        # add me to dest cell
        res ^= self.array[self.get_offset(board.piece_at(move.from_square)) + move.to_square]

        return res


    def update_hash(self, board:chess.Board, position_hash:int):
        return (position_hash ^ self.hash_castling(board) ^
                self.hash_ep_square(board) ^ self.hash_turn(board))
